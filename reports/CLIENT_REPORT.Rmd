...
```
Replacement possible claim: # Claim

{Yadavan} Across the multi-year dataset, the Treatment group consistently achieves a higher 30-day purchase rate than the Control group, demonstrating a stable positive trend that persists despite seasonal fluctuations.
...
within 30 days, for Treatment vs Control. The error bars reflect uncertainty
from the sample size and help indicate whether the difference is likely to be
due to random variation. {Yadavan} Because the error bars do not overlap, the difference between the 2 groups is statistically significant, so we can be confident that the higher purchase rate in the Treatment group is a real trend and not just a result of random monthly fluctuations."
...
**Interpretation.** This chart summarises whether one group
tends to have higher typical spend than the other. Differences here can matter
for interpretation, because higher-value customers may respond differently
regardless of the intervention. {Yadavan} We know from the first visualisation that the treatment group has a higher purchase rate but from this chart we see that the average order value is roughly the same, so this means the growth is coming from more people buying, not just people buying more expensive items.
